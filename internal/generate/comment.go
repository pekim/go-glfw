...
func (comment *comment) resolveRefs(text string) string {
	for {
		...
		} else if title, url, resolved := comment.resolveRef(ref); resolved {
...
func (comment *comment) resolveRef(ref string) (string, string, bool) {
...
/*
func (comment *comment) resolveRefs() {
	re := regexp.MustCompile(`@ref ([\w\d_]+)`)
	...
	for i, line := range comment.lines {
		...
		for parts != nil {
			ref := parts[1]
			if resolvedRef, resolved := comment.gen.resolveEntityReference(ref); resolved {
				if ref == "GLFW_INVALID_ENUM" {
					fmt.Println(strings.Replace(line, "@ref "+ref, "["+resolvedRef+"]", 1))
				}
				line = strings.Replace(line, "@ref "+ref, "["+resolvedRef+"]", 1)
				parts = re.FindStringSubmatch(line)

			} else if url, haveURL := comment.gen.entityURLs[ref]; haveURL {
				line = strings.Replace(line, parts[0], "["+ref+"]", 1)
				comment.links = append(comment.links, commentLink{
					text: ref,
					url:  url,
				})
				...
				// } else if url, haveURL := comment.gen.pageSectionURLs[ref]; haveURL {
				// 	fmt.Println(ref, url)
				// 	line = strings.Replace(line, parts[0], "["+ref+"]", 1)
				// 	comment.links = append(comment.links, commentLink{
				// 		text: ref,
...
func (comment *comment) createLinks() {
	...
	for i, line := range comment.lines {
		...
		for parts != nil {
			...
			if strings.HasPrefix(url, "[") {
				line = strings.Replace(line, match, url, 1)
			} else {
				line = strings.Replace(line, "("+url+")", "", 1)
				ref := strings.TrimPrefix(url, "@ref ")
				if pageSectionURL, haveURL := comment.gen.pageSectionURLs[ref]; haveURL {
					url = pageSectionURL
				}
				comment.links = append(comment.links, commentLink{
					text: text,
...
func (comment *comment) addCDocsLink() {
	url, haveURL := comment.gen.entityURLs[comment.entityName]
	if !haveURL {
		return
	}
	...
	insertIndex := len(comment.lines) // default to inserting at end of lines (if no heading found)
	if firstHeadingIndex != -1 {
		insertIndex = firstHeadingIndex - 1
		insertIndex = max(insertIndex, 0)
	}

	cDocsLines := []string{
		"",
		fmt.Sprintf("C documentation : [%s]", comment.entityName),
		"",
	}
	comment.lines = slices.Insert(comment.lines, insertIndex, cDocsLines...)

	comment.links = append(comment.links, commentLink{
		text: comment.entityName,
		url:  url,
	})
}
...
func (gen gen) commentLines(cursor clang.Cursor) []string {
	...
	previousLine := strings.TrimSpace(gen.lines[line-2])
	if !strings.HasPrefix(previousLine, "*") || !strings.HasSuffix(previousLine, "*/") {
		// There is no comment preceding the cursor
		return nil
	}
	...
	for l := int(line) - 1; l >= 0; l-- {
		if strings.HasPrefix(strings.TrimSpace(gen.lines[l]), "/*") {
			foundFirstLine = true
			firstLine = l
			break
		}
	...
	lines := make([]string, lastLine-uint32(firstLine))
	copy(lines, gen.lines[firstLine:lastLine])
	return lines
}
